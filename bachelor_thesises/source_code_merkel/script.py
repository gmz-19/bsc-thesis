...
class Sampling:
    ...
    def requestRepos(self):
        ...
        for single_date in sampling.daterange(self.startDate, self.endDate):
            ...
            Characteristics:
                ...
                4. Date range
            """
            url = f"https://api.github.com/search/repositories?q=stars:>=5+language:{self.curLanguage}+created:{currentDate}&sort=stars&order=asc&per_page=100"
            repoByLanguageResponse = requests.get(
                url, auth=("username", constants.TOKEN4)
            )
    ...
    def checkRepoByCharacteristics(self, firstRepoIndex, lastRepoIndex, numberOfRepos):
        ...
        Args:
            ...
            lastRepoIndex (integer): Up to this index, the raw repo list is examined.
        ...
        if not firstRepoIndex + lastRepoIndex <= len(repoList["repositories"]):
            sys.exit(
        ...
        for repo in repoList["repositories"]:
            if repo["index"] < firstRepoIndex:
                continue
            # Sort out repos that are trivially not an application
            if (
                repo["repoFullName"].endswith("js")
                or repo["repoFullName"].endswith("icons")
                or repo["repoFullName"].endswith("icon")
            ):
                lostRepos += 1
                firstRepoIndex += 1
                continue

            if reposWithcharacteristics["total_count"] <= numberOfRepos:
                # 1
                print(
                    "Repo "
                    + str(firstRepoIndex)
                ...
                print("checkIfApp")
                if (
                    sampling.checkIfApp(repo["repoFullName"])
                ...
                endTime = time.time()
                print(
                    "Time to check characteristics of "
                    + str(lastRepoIndex - startIndex)
                    + " Repos:"
                )
        ...
        while True:
            ...
            try:
                repoIssuesResponse = session.get(
                    issuesUrl, auth=("username", constants.TOKEN4)
                )
        ...
        while True:
            ...
            try:
                repoIssuesCommentsResponse = session.get(
                    issuesUrl, auth=("username", constants.TOKEN5)
                )
            except session.exceptions.ConnectionError:
                time.sleep(300)
    ...
    def getBugCommits(self, repoName):
        ...
        JSRepos = fileClass.openFile("JavaScriptReposCharacteristics.txt")

        df = pd.read_csv("JavaScriptReposCharacteristics.csv")

        print(df.loc[df["framework"] == "vue"])
        for repoName in df.loc[df["framework"] == "vue"]["repo_name"]:
            sampling.setErrorToDefault()
            timeOut = 0
            page = 0
            commitCount = 0
            ...
            while True:
                ...
                try:
                    repoCommitRequest = session.get(
                        commitUrl, auth=("username", tokenRequest[countRequest % 6])
                    )

                    if repoCommitRequest.status_code == 403:
                        print(
                            f"Status code 403. Sleep for 5 mins. Token for first request used (Need to have a look at array index): {countRequest % 6}"
                        )
                        time.sleep(300)
                        countRequest += 1
                        repoCommitRequest = session.get(
                        ...
                        if repoCommitRequest.status_code == 403:
                            print(
                                f"Still status code 403. Sleep for another 5 mins. Token for first request used (Need to have a look at array index): {countRequest % 6}"
                            )
                            time.sleep(300)
                            countRequest += 1
                            repoCommitRequest = session.get(
                                commitUrl,
                                auth=("username", tokenRequest[countRequest % 6]),
                            )
                        ...
                        for commit in repoCommitRequestJson:
                            ...
                            try:
                                ...
                                if repoCommitFilesRequest.status_code == 403:
                                    print(
                                        f"Status code 403. Sleep for 5 mins. Token for second request used (Need to have a look at array index): {countRequest % 6}"
                                    )
                                    countRequest += 1
                                    time.sleep(300)
                                    repoCommitFilesRequest = session.get(
                                        ...
                                        auth=(
                                            "username",
                                            tokenRequest[countRequest % 6],
                                        ),
                                    )
                                    if repoCommitFilesRequest.status_code == 403:
                                        print(
                                            f"Still status code 403. Sleep for another 5 mins. Token for second request used (Need to have a look at array index): {countRequest % 6}"
                                        )
                                        time.sleep(300)
                                        countRequest += 1
                                        repoCommitFilesRequest = session.get(
    ...
    def checkIfPackegeJson(self, repoName):
        ...
        branchRequest = requests.get(
            defaultBranchUrl, auth=("username", constants.TOKEN4)
        )
        ...
        if branchRequest.status_code == 200:
            ...
            treeRequest = requests.get(treeUrls, auth=("username", constants.TOKEN4))
    ...
    def lda(self, repoFullName):
        ...
        description = requests.get(descriptionUrl, auth=("username", constants.TOKEN4))

        descriptionJson = description.json()

        readme = requests.get(readmeUrl, auth=("username", constants.TOKEN4))

        # If README.md is not written in capital letters
        if readme.status_code == 404:
            readme = requests.get(readmeUrl2, auth=("username", constants.TOKEN4))
            if readme.status_code == 404:
                readme = requests.get(readmeUrl3, auth=("username", constants.TOKEN4))

        readmeText = readme.text

        print(readme.status_code)
        ...
        corpus = [dictionary.doc2bow(test) for token in tokens]
    ...
    def requestLanguages(self, repoName):
        ...
        return requests.get(url, auth=("username", constants.TOKEN4))
    ...
    def checkApiLimit(self, username):
        ...
        test = requests.head(url, auth=("username", constants.TOKEN2))
        print(test.headers)
...
class SonarQubeDance:
    ...
    def __init__(self, language):
        ...
        Args:
            language (string): The programming language for which the functions from this class should be executed.
        """
        self.curLanguage = Language(language)
            ...
            if len(frameworklists) == 2 and "express" in frameworklists:
                for frameworkEle in frameworklists:
                    if not frameworkEle == "express":
                        commaSeparatedValues.changeValueInCSV(
                            repo["index"], "framework", frameworkEle
                        )
            elif len(frameworklists) == 1:
                commaSeparatedValues.changeValueInCSV(
                    repo["index"], "framework", frameworklists[0]
                )
            elif len(frameworklists) > 1:
                print(
                ...
                commaSeparatedValues.changeValueInCSV(
                    repo["index"], "framework", "No framework identified"
                )
...
class CSV:
    ...
    def __init__(self, language, fileSuffix):
        """
        Args:
            language (string): The programming language for which the functions from this class should be executed.
        """
...
class PrintDiagramms:
    ...
    def boxplot(self, vals, metric, countDataSet, xTicks):
        ...
        Args:
            vals (list): List of js and ts vals.
            metric (string): Metrics can be: avg_bug-issue_time, code-smells_ncloc, cognitive-complexity_ncloc, bug-fix-commits_ratio.
            countDataSet (list): List of numbers of box plots ([1, 2, 3, ...])
            xTicks (list): List of labels of box plots (['JavaScript with react', 'TypeScript with react', 'JavaScript with angular', 'TypeScript with angular', ...])
        """
...
sampling = Sampling("TypeScript")

sonarQubeDance = SonarQubeDance("TypeScript")
...
cloneRepo = CloneRepo("TypeScript")

packageJson = PackageJson()

tsconfig = Tsconfig()

metrics = Metrics("TypeScript")
